...
from flask import request, redirect, url_for, flash, jsonify, make_response, g
...
def index():
    ...
    if request.method == 'POST':
        ...
        url_base = "https://d1apf2cvs5.execute-api.us-east-1.amazonaws.com/dev/letters/" + str(id) + "?pnum="
        urls = []
        ...
        for x in range(len(resultlist)):
            address = session.query(Address).\
                filter(Address.address == resultlist[x]).first()
            policy = session.query(Policy).\
                filter(Policy.id == address.policy_id).first()
            urls.append(url_base + str(policy.number))

        return jsonify(Url=urls)

    else:
        general = session.query(Letter).\
            filter(Letter.policy_type == "general").all()
        commercial = session.query(Letter).\
            filter(Letter.policy_type == "commercial").all()
        dwellers = session.query(Letter).\
            filter(Letter.policy_type == "dwellers fire").all()
        home = session.query(Letter).\
            filter(Letter.policy_type == "homeowners").all()
...
def letter(id, **kwargs):
    ...
    contact = session.query(Contacts).\
        filter(Contacts.id == policy.contact_id).first()
    address = session.query(Address).\
        filter(Address.policy_id == policy.id).first()
    address_heading = address.address.split(",", 1)
    policy_type = session.query(PolicyType).\
        filter(PolicyType.policy_id == policy.id).first()
    today = (str(date.today().strftime('%m/%d/%Y')))
    letter = session.query(Letter).filter(Letter.id == id).one()
    return render_template('letter_template.html',
                           id=id,
                           date=today,
                           letter=letter,
                           policy=policy,
...
def editLetter(id):
    ...
    html_template_string = """
        ...
        <section class="container">
          ...
          <p>Dear Insured: </p>
          <p>Our records indicate that under %s policy #
          %s we currently insure your dwelling
          located at the following address:</p>
          <p id="address">%s</p>
          %s
        <p>Respectfully,<br>The Farmers Fire Insurance Company </p>
    """ % (today,
           contact.first_name.title(),
           contact.last_name.title(),
           address_heading[0].title(),
...
def pdfLetter(id, **kwargs):
    ...
    rendered_template = render_template('pdf.html',
                                        date=today,
                                        letter=letter,
                                        policy=policy,
                                        contact=contact,
    ...
    pdffile = pdfkit.from_string(rendered_template,
                                 False,
                                 options=options,
                                 css=css,
...
def getPolicy(policy):
    try:
        ...
        if policy:
            ...
            return jsonify(Policy=[contact.serialize,
                                   address.serialize,
                                   policy.serialize,
                                   policy_type.serialize, "pn"])
...
def getContacts():
    try:
        ...
        if last_name is None:
            contacts = session.query(Contacts).\
                filter(Contacts.first_name == first_name).limit(100).all()
            return jsonify(Policy=iterContacts(contacts))

        else:
            contacts = session.query(Contacts).\
                filter(Contacts.first_name == first_name).\
                filter(Contacts.last_name == last_name).limit(100).all()
            return jsonify(Policy=iterContacts(contacts))
...
def getAddress(address):
    try:
        address = session.query(Address).\
            filter(Address.address == address).first()
        if address:
            contact = session.query(Contacts).\
                filter(Contacts.id == address.contact_id).first()
            policy = session.query(Policy).\
                filter(Policy.id == address.policy_id).first()
            policy_type = session.query(PolicyType).\
                filter(PolicyType.policy_id == address.policy_id).first()
            return jsonify(Policy=[contact.serialize,
                                   address.serialize,
                                   policy.serialize,
                                   policy_type.serialize,
                                   "add"])
...
def getPolicyType(ptype):
    try:
        ...
        policytype = session.query(PolicyType).\
            filter(PolicyType.policy_type == ptype).limit(100).all()
        if len(policytype) > 0:
            policyTypes = []
            for x in range(len(policytype)):
                policyinfo = session.query(Policy).\
                    filter(Policy.id == policytype[x].policy_id).first()
                contact = session.query(Contacts).\
                    filter(Contacts.id == policyinfo.contact_id).first()
                address = session.query(Address).\
                    filter(Address.policy_id == policyinfo.id).\
                    first()
                policy = {"first_name": contact.first_name,
                          "last_name": contact.last_name,
                          "address": address.address,
                          "policy": policytype[x].policy_type,
...
def advancedSearch():
    try:
        query_all = session.query(Contacts, Policy, Address, PolicyType).\
                            join(Policy, Policy.contact_id == Contacts.id).\
                            join(Address,
                                 and_(Address.contact_id == Contacts.id,
                                      Address.policy_id == Policy.id)).\
                            join(PolicyType, PolicyType.policy_id == Policy.id)
        arg_dict = {'first_name': request.args.get("fname"),
                    "last_name": request.args.get("lname"),
                    "address": request.args.get("address"),
                    "policy": request.args.get("pnum"),
                    "policytype": request.args.get("ptype")}
        truthiness = {}
        response = []
        ...
        valid_inputs = {k: v for k, v in truthiness.items() if v is True}
        ...
        if len(valid_keys) > 0:
            for x in range(len(valid_keys)):
                if(valid_keys[x] == 'first_name'):
                    query_all = query_all.\
                        filter(Contacts.first_name ==
                               request.args.get("fname"))

                if(valid_keys[x] == 'last_name'):
                    query_all = query_all.\
                        filter(Contacts.last_name == request.args.get("lname"))

                if(valid_keys[x] == 'address'):
                    query_all = query_all.\
                        filter(Address.address == request.args.get("address"))

                if(valid_keys[x] == 'policy'):
                    print "here"
                    query_all = query_all.\
                        filter(Policy.number == int(request.args.get("pnum")))

                if(valid_keys[x] == 'policytype'):
                    query_all = query_all.\
                        filter(
                            PolicyType.policy_type ==
                            str(request.args.get("ptype")))

            get_filtered_list = query_all.all()

            if len(get_filtered_list) > 0:
                for x in range(len(get_filtered_list)):
                    contact = {"id": get_filtered_list[x][0].id,
                               "first_name": get_filtered_list[x][0].
                               first_name,
                               "last_name": get_filtered_list[x][0].last_name,
                               "policy_number": get_filtered_list[x][1].number,
                               "address": get_filtered_list[x][2].address,
                               "policy": get_filtered_list[x][3].policy_type}
...
def iterContacts(contactList):
    ...
    if len(contactList) > 0:
        for x in range(len(contactList)):
            address = session.query(Address).\
                filter(Address.contact_id == contactList[x].id).all()
            print address
            addresslist = [a.address for a in address]
            print addresslist
            contact = {"id": contactList[x].id,
                       "first_name": contactList[x].first_name,
                       "last_name": contactList[x].last_name,
